    ...
    F: FnMut() -> Fut,
{
    ...
    loop {
        match f().await {
            ...
            Err(err) => {
                vlog::warn!("Failed s3 request {retries}/{max_retries}, retrying.");
...
impl AsyncS3Storage {
    pub async fn new(
        ...
        let client_config = retry(max_retries, || {
            Self::get_client_config(region, credential_file_path.clone())
        })
        .await
    ...
    async fn get_client_config(
        ...
        let provider_region = provider.region().await.unwrap();

        //TODO:
        //load credential_file_path as the account key and set to the client connection.
...
impl S3Storage {
    pub fn new(
        ...
        let inner = AsyncS3Storage::new(credential_file_path, bucket_prefix, max_retries);
        Self {
